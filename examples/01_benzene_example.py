...
def main():
    ...
    indexes[0] = np.column_stack((np.arange(len(baseline_vals)), np.arange(len(baseline_vals))))
    ...
    for i in range(num_fids - 1, -1, -1):
        ...
        fid_map = {row[0]: (row[1], y_trains[i][idx]) for idx, row in enumerate(indexes[i])}
