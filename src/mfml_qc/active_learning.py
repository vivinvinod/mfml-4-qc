...
def _train_estimator(base_estimator: object, X: np.ndarray, y: np.ndarray) -> object:
    ...
    if hasattr(model, 'train'):
        ...
        raise AttributeError("Estimator must have a '.train(X, y)' or '.fit(X, y)' method.")
    return model
...
def _uq_variance(model: object, X_pool: np.ndarray, X_train: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        if hasattr(model, '_generate_kernel') and hasattr(model, 'reg'):
            ...
            raise TypeError("Estimator does not support 'return_std=True' and is not a recognized KRR class.") 

def _uq_ensemble(base_estimator: object, X_pool: np.ndarray, X_train: np.ndarray, y_train_primary: np.ndarray, n_ensemble: int = 5) -> np.ndarray:
    """Helper for Ensemble-based uncertainty quantification."""
    pool_preds = np.zeros((X_pool.shape[0], n_ensemble), dtype=float)
    for n in range(n_ensemble):
        X_sub, _, y_sub, _ = train_test_split(X_train, y_train_primary, train_size=0.85, random_state=n)
...
def _uq_lfab(base_estimator: object, X_pool: np.ndarray, y_pool: Union[Dict, np.ndarray], X_train: np.ndarray, y_train: Union[Dict, np.ndarray]) -> np.ndarray:
    ...
    if not isinstance(y_train, dict) or 'low' not in y_train or 'low' not in y_pool:
        raise ValueError("LFaB UQ requires `y_train` and `y_pool` to be dictionaries containing a 'low' key.")
...
def active_learning_loop(X_train: np.ndarray, y_train: Union[np.ndarray, Dict[str, np.ndarray]],
                         X_pool: np.ndarray, y_pool: Union[np.ndarray, Dict[str, np.ndarray]],
                         base_estimator: object,
                         uq_mode: Union[str, Callable] = 'variance',
                         al_iters: int = 500,
                         X_test: np.ndarray = None, y_test: Union[np.ndarray, Dict[str, np.ndarray]] = None,
                         p_bar: bool = True,
                         **uq_kwargs) -> dict:
    """
    ...
    **uq_kwargs : dict
    ...
    for _ in tqdm(range(al_iters), desc=f'AL Loop ({uq_mode})', disable=not p_bar):
        ...
        if callable(uq_mode):
            uncertainty = uq_mode(model, X_pool, y_pool, base_estimator, X_train, y_train, **uq_kwargs)
        elif uq_mode == 'variance':
            uncertainty = _uq_variance(model, X_pool, X_train)
        elif uq_mode == 'ensemble':
            uncertainty = _uq_ensemble(base_estimator, X_pool, X_train, y_train_primary, **uq_kwargs)
        elif uq_mode == 'lfab':
            uncertainty = _uq_lfab(base_estimator, X_pool, y_pool, X_train, y_train)
        else:
            raise ValueError(f"Unknown uq_mode: {uq_mode}. Use 'variance', 'ensemble', 'lfab', or pass a custom callable.")
