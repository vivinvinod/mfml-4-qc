...
def test_active_learning_loop_mechanics(standard_data):
    ...
    assert results['training_sizes'][-1] == initial_train_size + iters - 1 # starts at iteration 0
...
def test_active_learning_loop_lfab(lfab_data):
    ...
    results = active_learning_loop(
        X_train.copy(), copy.deepcopy(y_train), 
        X_pool.copy(), copy.deepcopy(y_pool),
        base_estimator=DummyKRRModel(),
        uq_mode='lfab',
        al_iters=iters,
        X_test=X_test, y_test=y_test, # Passing test set should trigger MAE
        p_bar=False
    )
